...
// Importações de imagens (Os paths são resolvidos corretamente pelo Vite)
...
// ====================================================================
// FUNÇÃO DE LIMPEZA PARA REMOVER O HASH DO VITE
// ====================================================================
const cleanAvatarName = (avatarString) => {
    if (!avatarString) return null;
    let name = avatarString.trim();

    // Regex para identificar e remover o hash do Vite antes da extensão.
    // Ex: 'bode-Yp4MO_tb.svg' -> 'bode.svg'
    const viteHashPattern = /(\-[a-zA-Z0-9]+)(\.[^/.]+)$/;
    const cleanedName = name.replace(viteHashPattern, '$2');
    
    // Retorna o nome limpo (ex: 'bode.svg') ou o nome original se não tiver hash.
...
    // ====================================================================
    // NOVA LÓGICA DO AVATAR SRC
    // ====================================================================
    const avatarNameFromBackend = user?.avatar;
    
    // 1. Limpa o hash do Vite (ex: "bode-Yp4MO_tb.svg" -> "bode.svg")
    const cleanedNameWithExt = cleanAvatarName(avatarNameFromBackend);
    
    // 2. Tenta encontrar a chave no mapa usando:
    //    a) O nome limpo com a extensão ("bode.svg")
    //    b) O nome limpo sem a extensão ("bode")
