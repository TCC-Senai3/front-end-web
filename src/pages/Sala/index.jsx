...
export default function Sala() {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (
        ...
        event.target.closest('.sala-actions') ||
        event.target.closest('.confirm-modal-content')
      ) {
        return;
      }
  ...
  useEffect(() => {
        ...
        client.subscribe(topic, (message) => {
          ...
          try {
            ...
            if (payload.type === "JOGO_INICIADO") {
              ...
              setActionLoading(false); // Reseta o loading antes de navegar
              navigate("/jogo", {
    ...
    // Verifica se já está conectado após um pequeno delay
    // Isso garante que se o STOMP já estiver ativo, o estado será atualizado
    ...
    return () => {
      ...
      onConnectCalledRef.current = false;
...
useEffect(() => {
  ...
  const handleIniciar = async () => {
    ...
    // Verifica se o cliente STOMP está disponível
    ...
    // Verifica se está realmente conectado (não apenas ativo)
    const ws = client.webSocket;
    const isWebSocketOpen = ws && ws.readyState === WebSocket.OPEN;
    const isReallyConnected = isConnected || isWebSocketOpen || (client.state === 1);
    
    if (!isReallyConnected) {
      // Tenta ativar se não estiver ativo
      if (!client.active) {
        client.activate();
      }
      // Aguarda um pouco e tenta novamente
      await new Promise(resolve => setTimeout(resolve, 1000));
      const wsAfterWait = client.webSocket;
      const isWebSocketOpenAfterWait = wsAfterWait && wsAfterWait.readyState === WebSocket.OPEN;
      if (!isWebSocketOpenAfterWait && !client.active) {
        return;
      }
    }
    ...
    // CRÍTICO: Aguarda o onConnect ser chamado antes de publicar
    // O STOMP precisa completar o handshake antes de poder publicar
    let attempts = 0;
    const maxAttempts = 20; // 10 segundos no total
    
    while (!onConnectCalledRef.current && attempts < maxAttempts) {
      // Verifica se já está conectado mesmo sem o onConnect ter sido chamado
      const currentWs = client.webSocket;
      if (currentWs && currentWs.readyState === WebSocket.OPEN && client.active) {
        onConnectCalledRef.current = true;
        setIsConnected(true);
        break;
      }
    ...
    try {
      // Verifica novamente antes de publicar
      const finalWs = client.webSocket;
      if (!finalWs || finalWs.readyState !== WebSocket.OPEN) {
        ...
        // Verifica novamente
        const finalWs2 = client.webSocket;
        if (!finalWs2 || finalWs2.readyState !== WebSocket.OPEN) {
          throw new Error("Conexão não disponível");
        }
      }
      
      // Verifica se o cliente STOMP está realmente pronto para publicar
      if (!client.active) {
        throw new Error("Cliente STOMP não está ativo");
      }
      
      const destination = `/app/sala/${codigo}/iniciar`;
      const messageBody = JSON.stringify({ idUsuario: user.id });
      
      // Publica a mensagem para iniciar o jogo
      client.publish({
        destination,
        body: messageBody,
      });
      
      // O loading será resetado quando:
      // 1. O jogo iniciar (via mensagem JOGO_INICIADO no WebSocket)
      // 2. Houver um erro (catch abaixo)
            ...
            {isDonoDaSala && (
              <button
                ...
                disabled={
                  ...
                  usuarios.length < 1 || 
                  (!isConnected && 
                   !(stompClientRef.current?.webSocket?.readyState === WebSocket.OPEN) && 
                   stompClientRef.current?.state !== 1)
                }
                type="button"
