...
export default function GameQuiz({ quizData, codigoSala, idSala }) {
  ...
  const [timeLeft, setTimeLeft] = useState(15); // <<< AJUSTADO (era 90)
  ...
  useEffect(() => {
    ...
    if (quizData && quizData.perguntas && quizData.perguntas.length > 0) {
      ...
      setScore(0);
      // Prioriza o tempo do quizData, senão usa 15s
      const tempoSegundos = quizData.tempoLimite ? quizData.tempoLimite * 60 : 15; // <<< AJUSTADO (era 90)
  ...
  const handleAnswerSelect = async (alternativa) => {
    ...
    // Calcula o tempo gasto baseado no tempo limite (prioriza pergunta, quiz, ou 15s)
    const tempoLimitePergunta = currentQuestion.tempo || quiz.tempoLimite * 60 || 15; // <<< AJUSTADO (era 90)
  ...
  const handleNext = useCallback(() => {
    ...
    if (quiz && proximoIndex < quiz.perguntas.length) {
      ...
      // Reseta o tempo para a próxima pergunta (prioriza pergunta, quiz, ou 15s)
      const tempoSegundos = quiz.perguntas[proximoIndex].tempo || quiz.tempoLimite * 60 || 15; // <<< AJUSTADO (era 90)
      setTimeLeft(tempoSegundos);
    } else {
  ...
  useEffect(() => {
    if (showResultScreen) {
      const timer = setTimeout(() => {
        handleNext();
      }, 4000); // Tempo de exibição da tela de resultado
